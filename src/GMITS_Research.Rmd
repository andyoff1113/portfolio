---
title: "GMITS: Grey Model for Irregular Time Series"
subtitle: "Research Overview — Early-Stage Outbreak Forecasting"
author: "Andy Chu"
date: "2026-05-23"
---
...
GMITS extends the classical grey-system GM(1,1) model to irregularly
spaced reporting intervals, aimed at early-stage outbreak forecasting
when only a handful of data points are available. This page collects
the shared methodology and applies it to two real outbreaks — see
[Bongkrekic Acid Food Poisoning (2024)](Bongkrekic_Acid_2024.html) and
[2011 Listeria Outbreak](Listeria_2011.html) for the standalone
write-ups of each.
...
The most recent observation in the n = 4 Listeria input (Aug 23, count = 1)
may be underreported due to reporting delay. We scale it by 1.5× and 2.0× and
...
Ground truth for Bongkrek: `Tracking_4_07b` at the first forecast step (3/24).
Ground truth for Listeria: `082712A` at all 4 forecast steps.

```{r error-summary}
# ---- Bongkrek: 1 verifiable step (3/24) ----
...
# ---- Listeria n=6: 4 verifiable steps ----
fc_n6 <- df_lst_fc |>
...
bind_rows(
  data.frame(
    ...
    n_input   = 4L,
    n_steps   = 1L,
    MAE_oos   = abs(bong_fc_step1_g  - bong_gt_step1),
    MAPE_oos  = abs(bong_fc_step1_g  - bong_gt_step1) /
                  bong_gt_step1 * 100
  ),
  data.frame(
    Dataset   = "Bongkrek",
    ...
    n_input   = 4L,
    n_steps   = 1L,
    MAE_oos   = abs(bong_fc_step1_er - bong_gt_step1),
    MAPE_oos  = abs(bong_fc_step1_er - bong_gt_step1) /
                  bong_gt_step1 * 100
  ),
  data.frame(
    Dataset   = "Listeria (n = 6)",
    Model     = "GMITS",
    n_input   = 6L,
    n_steps   = nrow(lst_err),
    MAE_oos   = mae(lst_err$Count_gt, lst_err$Point),
    MAPE_oos  = mape(lst_err$Count_gt, lst_err$Point)
  ),
  data.frame(
    Dataset   = "Listeria (n = 6)",
    Model     = "Exp. Reg.",
    n_input   = 6L,
    n_steps   = nrow(lst_err),
    MAE_oos   = mae(lst_err$Count_gt, lst_err$ExpReg),
    MAPE_oos  = mape(lst_err$Count_gt, lst_err$ExpReg)
  )
) |>
  kable(
    digits    = 2,
    col.names = c("Dataset", "Model", "n input",
                  "Steps verified", "MAE (oos)", "MAPE % (oos)"),
    caption   = "Out-of-sample error: GMITS vs. exponential regression"
  ) |>
  kable_styling(bootstrap_options = c("striped", "hover")) |>
  pack_rows("Bongkrek (1 verifiable step)", 1, 2) |>
  pack_rows("Listeria 2011 (4 verifiable steps)", 3, 4)
```
